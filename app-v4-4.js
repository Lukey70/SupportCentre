...
const SESSION_TOKEN_KEY = "supportRequestPortal.supabase.agentToken.v4.4";
const SESSION_AGENT_KEY = "supportRequestPortal.supabase.agent.v4.4";
...
function renderInboxFromCache() {
  ...
  requestList.innerHTML = filtered.map((item) => `
    <button type="button" class="request-item case-card-item ${item.id === selectedAgentCaseId ? "active" : ""}" data-case-id="${escapeHtml(item.id)}">
      ...
      <span class="case-card-status-line">Status: ${escapeHtml(displayStatus(item.status))}${item.assigned_agent_display ? ` · Assigned to ${escapeHtml(item.assigned_agent_display)}` : " · Unassigned"}</span>
    </button>
  `).join("");
