...
const SESSION_TOKEN_KEY = "supportRequestPortal.supabase.agentToken.v4.5";
const SESSION_AGENT_KEY = "supportRequestPortal.supabase.agent.v4.5";
...
async function handleSupportSubmit(event) {
  ...
  const customerName = getTrimmedValue("requester-name");
  ...
  const validationMessage = validateSupportRequestForm({ customerName, customerEmail, selectedCategory, subject, details });
  if (validationMessage) {
    showToast(validationMessage);
    focusFirstBlankSupportField({ customerName, customerEmail, selectedCategory, subject, details });
    return;
  }
...
function renderCustomerCaseSummary() {
  ...
  customerRequestList.innerHTML = `
    <button type="button" class="request-item active">
      ...
      <span>${escapeHtml(displayStatus(selectedCustomerCase.status))} · ${escapeHtml(selectedCustomerCase.category)}</span>
    </button>
  `;
}
...
async function refreshInboxIfLoggedIn() {
  ...
  try {
    const data = await rpc("list_agent_cases", {
      ...
      p_category_filter: normaliseCategoryFilter(categoryFilter.value),
    });
...
function renderInboxFromCache() {
  ...
  requestList.innerHTML = filtered.map((item) => `
    <button type="button" class="request-item case-card-item ${item.id === selectedAgentCaseId ? "active" : ""}" data-case-id="${escapeHtml(item.id)}">
      <span class="case-card-topline">
        <span class="request-number case-card-number">${escapeHtml(item.case_number)}</span>
        <span class="status-pill case-list-status-pill ${statusClass(item.status)}">${escapeHtml(displayStatus(item.status))}</span>
      </span>
      <span class="case-card-subject-line"><strong>${escapeHtml(item.subject || "Untitled request")}</strong> <span class="case-card-customer">— ${escapeHtml(item.customer_name || "Unknown customer")}</span></span>
      <span class="case-card-category-line">Category: ${escapeHtml(item.category || "Uncategorised")}</span>
      <span class="case-card-status-line">${item.assigned_agent_display ? `Assigned to ${escapeHtml(item.assigned_agent_display)}` : "Unassigned"}</span>
    </button>
  `).join("");
...
function renderAgentCaseDetail() {
  ...
  requestDetail.innerHTML = `
    <div class="detail-header">
      ...
      <label class="status-badge-select-label" aria-label="Change case status">
        <span class="sr-only">Status</span>
        <select id="agent-status-select" class="status-pill status-select-pill ${statusClass(c.status)}" ${isClosed ? "disabled" : ""} title="Change status">
          ${["New", "In Progress", "Waiting on Customer", "Resolved", "Closed"].map((status) => `<option value="${status}" ${status === c.status ? "selected" : ""}>${displayStatus(status)}</option>`).join("")}
        </select>
      </label>
    </div>
    <div class="status-actions top-action-row" aria-label="Status actions">
      <div class="left-actions">
        <button id="start-work" type="button" class="secondary-btn" ${isStartWorkDisabled(c) ? "disabled" : ""}>${isStartWorkDisabled(c) ? "Work Started" : "Start Work"}</button>
        <button id="awaiting-info-case" type="button" class="secondary-btn" ${isClosed || isResolved || c.status === "Waiting on Customer" ? "disabled" : ""}>Awaiting Info</button>
  ...
  $("awaiting-info-case")?.addEventListener("click", handleAwaitingInfo);
...
function isStartWorkDisabled(caseData) {
  if (!caseData) return true;
  return caseData.status === "Closed"
    || caseData.status === "Resolved"
    || caseData.status === "In Progress"
    || caseData.status === "Waiting on Customer"
    || Boolean(caseData.assigned_agent_id);
}

function displayStatus(status) {
  return status === "Waiting on Customer" ? "Awaiting Info" : String(status || "");
}

function formatAgentDisplayName(agent) {
  if (!agent) return "Support Agent";
  const first = String(agent.first_name || "").trim();
  const lastInitial = String(agent.last_name || "").trim().charAt(0);
  return [first, lastInitial].filter(Boolean).join(" ") || "Support Agent";
}
...
async function handleStartWork() {
  if (!selectedAgentCase || isStartWorkDisabled(selectedAgentCase)) return;
  try {
...
async function handleAwaitingInfo() {
  if (!selectedAgentCase || selectedAgentCase.status === "Closed" || selectedAgentCase.status === "Resolved") return;
  try {
    await rpc("agent_change_status", { p_token: currentToken, p_case_id: selectedAgentCase.id, p_status: "Waiting on Customer" });
    await refreshInboxIfLoggedIn();
    showToast("Case set to Awaiting Info.");
  } catch (error) { showToast(error.message); }
}
...
async function handleAgentCategoryChange(event) {
  try {
    await rpc("agent_change_category", { p_token: currentToken, p_case_id: selectedAgentCase.id, p_category: normaliseCategoryForBackend(event.target.value) });
    await refreshInboxIfLoggedIn();
...
function openReplyModal(caseData) {
  ...
  const agentName = currentAgent ? formatAgentDisplayName(currentAgent) : "Support Agent";
...
function validateSupportRequestForm({ customerName, customerEmail, selectedCategory, subject, details }) {
  if (!customerName) return "Please enter your name before submitting.";
...
function focusFirstBlankSupportField({ customerName, customerEmail, selectedCategory, subject, details }) {
  if (!customerName) return $("requester-name")?.focus();
...
function normaliseCategoryFilter(value) {
  const clean = String(value || "").trim();
  if (!clean || clean === "All" || clean === "All Categories") return "All Categories";
  return normaliseCategoryForBackend(clean);
}
...
function openModal(id) {
  const modal = $(id);
  if (!modal) {
    showToast("Popup could not be opened. Refresh the page and try again.");
    return;
  }
