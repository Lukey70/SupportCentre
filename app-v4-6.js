...
const SESSION_TOKEN_KEY = "supportRequestPortal.supabase.agentToken.v4.6";
const SESSION_AGENT_KEY = "supportRequestPortal.supabase.agent.v4.6";
...
function openReplyModal(caseData) {
  ...
  replyBody.value = `Hello ${getCustomerFirstName(caseData)},\n\n\n\nKind regards,\n${agentName}`;
...
function getCustomerFirstName(caseData) {
  const explicitFirstName = String(caseData?.customer_first_name || "").trim();
  if (explicitFirstName) return explicitFirstName;
  const fullName = String(caseData?.customer_name || "Customer").trim();
  return fullName.split(/\s+/)[0] || "Customer";
}
