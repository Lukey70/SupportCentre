...
async function handleSupportSubmit(event) {
  ...
  if (!selectedCategory) {
    showToast("Please select ICT, Finance Service, or Human Resources before submitting.");
    $("request-category")?.focus();
    return;
  }
  ...
  try {
    ...
    const data = await rpc("create_support_request", {
      p_customer_name: String(form.get("requesterName") || "").trim(),
      p_customer_email: String(form.get("requesterEmail") || "").trim(),
      p_category: selectedCategory,
      p_subject: String(form.get("subject") || "").trim(),
      p_description: String(form.get("details") || "").trim(),
    });
